...
(function() {
  function NgTuyenSonProtect() {
    ...
    var NgTuyenSonContent = "-Nguyen Tuyen Son-";
    var NguyenTSon = document.getElementById("Facebook_ng.tuyen.son.deptraivcl");
    if (NguyenTSon === null) {
      alert('Phát hiện source code đã bị chỉnh sửa - error 0x000001 !!!')
      window.location.replace("//facebook.com/ng.tuyen.son.deptraivcl");
      return;
    } else if (NguyenTSon.innerHTML !== NgTuyenSonContent){
      return;
    }
  }
  NgTuyenSonProtect();
...
// ABOUT ME
document.getElementById("NTSAbout_1").innerHTML = 'Nguyen Tuyen Son - Ho Tan Son is the most handsome man in the world';
// FOOTER
const footerElement = document.createElement("footer");
footerElement.classList.add("Cuoi", "Trang");
const pElement = document.createElement("p");
pElement.classList.add("NgTuyenSonFtCopyright");
const spanElement = document.createElement("span");
spanElement.style.fontFamily = "Monomaniac One";
spanElement.classList.add("copyright");
const aElement = document.createElement("a");
aElement.style.textDecoration = "none";
aElement.href = "//facebook.com/ng.tuyen.son.deptraivcl";
const uElement = document.createElement("u");
uElement.style.color = "aliceblue";
uElement.textContent = "HsonCuti ♡";
...
.MangXaHoiFtNts:after {
  content: "";
  clear: both;
  display: table;
}
.MangXaHoiFtNts div:not(:last-child) {
  border-right: none; 
}
.MangXaHoiFtNts div:hover {
...
// USER NAME + TÍCH XANH
document.getElementById("userName").innerHTML = 'Nguyễn Tuyển Sơn <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e4/Twitter_Verified_Badge.svg/512px-Twitter_Verified_Badge.svg.png" width="17" height="17" alt="Tích Xanh">';
// CHECK FPS
...
let typed = new Typed(".GioiThieu", {
  strings: ["Anh đẹp trai không? "," My Name is Nguyễn Tuyển Sơn :))"],
  typeSpeed: 95,
  backSpeed: 75,
  loop: true
})
// MUSIC PLAY
const Nts_List = [
"//files.catbox.moe/46m1sx.mp3", // thủ đô cypher
"//files.catbox.moe/i845t1.mp3", // hôm nay em cưới rồi
];
let index = parseInt(localStorage.getItem('currentAudioIndex')) || Math.floor(Math.random() * Nts_List.length);
const audio = new Audio(Nts_List[index]);
function hsonAudio() {audio.play();}
document.addEventListener('click', hsonAudio);
audio.addEventListener("ended", function() {
index = (index + 1) % Nts_List.length;
localStorage.setItem('currentAudioIndex', index);
audio.src = Nts_List[index];
audio.play();
});
function onCreate() {
  CheckFps();
...
function Youtube() {
  setTimeout(function() {
    window.open('https://www.youtube.com/channel/UC87xxZ0l6NDKPQTCsy9xeCg', '_blank')},
  100);
}
function Facebook() {
  setTimeout(function() {
    window.open('https://www.facebook.com/ng.tuyen.son.deptraivcl/', '_blank')},
  100);
}
function Discord() {
  setTimeout(function() {
    window.open('https://discord.com/channels/@HsonCuti', '_blank')},
  100);
}
function Instagram() {
  setTimeout(function() {
    window.open('https://instagram.com/profilexzz.006/', '_blank')},
  100);
}
function HomeNguyenTuyenSon() {
  Swal.fire({
    title: 'HACKER-NTS !',
    text: '=>> Nguyễn Tuyển Sơn 🐬 <<=',
    showConfirmButton: false
  });
}
const toastDiv = document.createElement('div');
toastDiv.id = 'Toast';
toastDiv.textContent = '𝘿𝙚𝙨𝙞𝙜𝙣 𝙗𝙮 𝘽𝙤̂́ 𝙈𝙖̀𝙮 𝙇𝙖̀ 𝙃𝙨𝙤𝙣 🚀';
